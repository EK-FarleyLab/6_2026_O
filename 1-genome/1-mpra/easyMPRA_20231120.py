...
def dict_step4_filter_dictionary(bc2en2count,
    ...
    fig,ax=plt.subplots(1,dpi=150)
    ax.scatter(plotu,plotmm,color=plotcolors,s=5)
    # ax.set_yscale('log')
    # ax.set_xscale('log')
    ax.axhline(maxMmReadsAcceptable,ls='--',lw=.1)
    ax.axvline(minUReadsRequired   ,ls='--',lw=.1)
    ax.set_ylabel('Secondary En Counts')
    ax.set_xlabel('Primary En Counts')
    ax.set_title('Multimatch Read Discrepansy')
    plt.show()
