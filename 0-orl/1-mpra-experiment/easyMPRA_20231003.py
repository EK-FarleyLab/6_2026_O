...
def read2count_tsv_to_dict(tsv):
    '''Converts read2count from tsv to python dictionary.'''
    read2count={}
    for read,count in read_tsv(tsv,pc=False,header=False):
        read2count[read]=float(count)
    return read2count

def  readsWithEnhancerAndBarcode_tsv_to_dict(tsv):
    '''Converts readsWithEnhancerAndBarcode from tsv to list of tuples.'''
    readsWithEnhancerAndBarcode=[]
    for en,bc,rc in read_tsv(tsv,pc=False,header=False):
        readsWithEnhancerAndBarcode.append((en,bc,rc))
    return readsWithEnhancerAndBarcode

def bc2en2count_tsv_to_dict(bc2en2count_tsv):
...
def ubc2en_tsv_to_dict(ubc2en_tsv):
...
def enhancerOrderSheet_tsv_to_dic(enOrderSheet):
...
def dict_step1_raw_to_collapsed_reads(rawReadFile,
    ...
    if printProgress:
        if fileType=='fastq':
            if not fastqTotalLines:
                ...
                print(f'File size of {in_fasq} specified by user to be...')
    else:
    ...
    print(f'Number reads excluded due to containing N:   {numReadsWithN:,}')
    print(f'Number reads excluded due to being too long: {numReadsTooLong:,}')
    print()
    n=len(read2count)
    print(f'Number Unique Reads OK: {n:,}')

    n=sum(read2count.values())
    print(f'Number Reads OK:        {n:,}')
...
def dict_step2SR_parse_en_bc_from_read(enOrderSheet,
    ...
    EnOrderName2Seq,EnOrderSeq2Name=enhancerOrderSheet_tsv_to_dic(enOrderSheet)
    if returnRevCompEn:
...
def dict_step2LR_parse_en_bc_from_read(enOrderSheet,read2count,bcLength,enLinker5,enLinker3,bcLinker5,bcLinker3,outfn,returnRevCompBc,returnRevCompEn):

    '''Converts raw reads into a tsv containing the parsed enhancer and barcode within the read.
    Saves new tsv file.'''
    ...
    ####################################################
    # Parse en and bc from reads
    ####################################################
    
    n_enLinkerDetected=0
    n_enDetectedInRead=0
    n_bcenLinkersDetected=0
    ...
    for read,count in read2count.items():
        ...
        ##############################################################
        # Check for en linkers
        ##############################################################
        
        # Is linker 3 in fwd sequence?
        if enLinker3 in read: 
            strand='+'
        
        else:
            
            # Is linker 3 in reverse sequence?
            read=revcomp(read)
            if enLinker3 in revcomp(read): 
                strand='-'
                
            # Is linker in neither fwd or reverse?
            else:
                continue
                

        # Is linker 5 detected?
        if enLinker5 in read:
    ...
    print(f'{n_enLinkerDetected:,}              reads with enhancers parsed...')
    print(f'of these, {n_enDetectedInRead:,}    reads with exact match to ordered en...')       
    print(f'of these, {n_bcenLinkersDetected:,} reads with barcodes parsed...')
    print()
    
    # Check bc length distribution
    data=[len(bc) for en,bc,count in readsWithEnhancerAndBarcode]
    fig,ax=plt.subplots(1,dpi=150)
    xmin=bcLength-10
    xmax=bcLength+10
    plt.hist(data,bins=range(xmin,xmax))
    plt.xlabel('Barcode Length')
    plt.title('Barcode Length Distribution')
...
def dict_step3_assemble_dict(readsWithEnhancerAndBarcode,enOrderSheet,outDictBasename):
    ...
    print(f'Of {len(orderedEns):,} enhancers ordered, {nMissingEnhancers:,} ({percent(nMissingEnhancers/len(orderedEns))}) are missing')
    ...
    # if multiple match, show discrepency in readcount from top enhancer to second
        ...
        print(f'\n{len(x)} Multiple matches detected in the raw dictionary (before filtering).\n\tThe multi-match most frequent enhancer vs second enhancer is plotted below:\n ')
    ...
    ##############################################################################
    print()
    print('=> The following dictionaries outputted here:')
    line_out=''
...
def dict_step4_filter_dictionary_for_unique_and_min_reads(bc2en2count,minReads,outDictBasename,bcTrimLength=None):
    ...
    if not bcTrimLength: print(f'Trimming barcodes to {bcTrimLength}...')
...
def exp_1_count_barcodes(in_fastq,out_path,bcLength,out_basename=None,progressReport=1e6,fastqTotalLines=None):
...
def exp_2_create_barcode_table(SampleName2Data, enOrderSheet, ubc2en, outbasename=None):
    ...
    _,En2EnId=enhancerOrderSheet_tsv_to_dic(enOrderSheet)
    ...
    for sampleName,Bc2ReadCount in SampleName2Data.items():
